            ...
            {  
                ...
                else
                {
                    ...
                    foreach (var File in review.FileResults)
                    {
                        ...
                        if (File.Approved)
                        {
                            var source = _minioTreHelper.GetCopyObject(review.OutputBucket, $"/{review.SubId}/" + File.FileName);
                            var resultcopy = _minioSubHelper.CopyObjectToDestination(bucket.Bucket, $"/{review.SubId}/" + File.FileName, source.Result).Result;
                        }
                    }
                }
