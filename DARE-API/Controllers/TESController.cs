...
namespace DARE_API.Controllers
{
    ...
    public class TESController : ControllerBase
    {
            ...
            private static readonly Dictionary<TesView, JsonSerializerSettings> TesJsonSerializerSettings = new()
        {
            ...
            public TaskServiceApiController(IRepository<TesTask> repository)
            {
            ...
            public virtual async Task<IActionResult> CancelTask([FromRoute][Required] string id, CancellationToken cancellationToken)
            {
                ...
                if (await repository.TryGetItemAsync(id, cancellationToken, item => tesTask = item))
                {
                    ...
                    {
                        Log.Information("{Function} Task {id} cannot be canceled because it is in {State} state.", "CancelTask", id, tesTask.State);
                    }
                    else if (tesTask.State != TesState.CANCELEDEnum)
                    {
                        Log.Information("{Function} Canceling task {id}", "CancelTask", id);
            ...
            public virtual async Task<IActionResult> CreateTaskAsync([FromBody] TesTask tesTask, CancellationToken cancellationToken)
            {
                ...
                if (tesTask?.Resources?.BackendParameters is not null)
                {
                    ...
                    {
                        Log.Warning("{Function }Unsupported keys were passed to TesResources.backend_parameters: {Keys}", "CreateTaskAsync",string.Join(",", unsupportedKeys));
                    }
                ...
                Log.Debug("{Function} Creating task with id {Id} state {State}", "CreateTaskAsync", tesTask.Id, tesTask.State);
            ...
            private async ValueTask<bool> TryRemoveItemFromCacheAsync(TesTask tesTask, string view, CancellationToken cancellationToken)
            {
                ...
                catch (Exception exc)
                {
                    ...
                    Log.Warning(exc, "{Function} An exception occurred while trying to remove TesTask with ID {Id} with view {view} from the cache", "TryRemoveItemFromCacheAsync", tesTask.Id, view);
                }
            ...
            private IActionResult TesJsonResult(object value, string view)
            {
                ...
                {
                    Log.Error("{Function }Invalid view parameter value. If provided, it must be one of: {Names}","TesJsonResult", string.Join(", ", Enum.GetNames(typeof(TesView))));
